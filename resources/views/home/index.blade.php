...
@section('content')
    {{-- Hero Section --}}
    <section class="relative h-screen flex items-center justify-center bg-[#3B1E0E] bg-center overflow-hidden" style="background-image: url('{{ asset('images/hero-section.png') }}')">
        <div class="absolute inset-0 bg-black opacity-70"></div>
        <div class="relative z-10 text-center px-4" data-aos="fade-up">
            {{-- <h1 class="text-5xl md:text-8xl font-bold text-white mb-4 tracking-tighter">CAFFEINE 1922</h1> --}}
            <img src="{{ asset('images/logo-2.png') }}" alt="{{ config('app.name') }}" class="mx-auto w-full">
            <p class="text-xl md:text-2xl text-[#F0E0C8] mb-8 italic">{{ \App\Models\Setting::get('tagline_' . app()->getLocale()) }}</p>
            <div class="flex flex-col md:flex-row justify-center gap-4">
                <a href="{{ route('menu') }}" class="bg-[#C8922A] text-white px-8 py-4 rounded-full font-bold hover:bg-[#B07D1E] transition">{{ __('site.cta.explore_menu') }}</a>
                <a href="{{ route('reserve') }}" class="bg-transparent border-2 border-white text-white px-8 py-4 rounded-full font-bold hover:bg-white hover:text-[#3B1E0E] transition">{{ __('site.cta.reserve_table') }}</a>
            </div>
        </div>
    </section>
    ...
    <section class="py-20 bg-[#FDF6EC]">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
                <div data-aos="{{app()->getLocale() === 'en' ? 'fade-right' : 'fade-left'}}">
                    ...
                    <a href="{{ route('about') }}" class="text-[#C8922A] font-bold hover:underline">{{ __('site.cta.our_story') }} →</a>
                </div>
                <div class="bg-[#F0E0C8] bg-center bg-cover h-96 rounded-lg shadow-xl" style="background-image: url({{ asset('images/story.png') }})" data-aos="{{app()->getLocale() === 'en' ? 'fade-left' : 'fade-right'}}">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-8 text-center">
                ...
                <div data-aos="zoom-in" data-aos-delay="100">
                    <div class="text-4xl font-bold text-[#C8922A] mb-2">{{ \App\Models\Setting::get('rating') }}★</div>
                    <div class="text-sm uppercase tracking-widest">Google Rating</div>
                </div>
                <div data-aos="zoom-in" data-aos-delay="200">
                    <div class="text-4xl font-bold text-[#C8922A] mb-2">{{ \App\Models\Setting::get('reviews_count') }}+</div>
    ...
    <section class="py-20 bg-[#FDF6EC]">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="text-center mt-8">
                <a href="{{ route('menu') }}" class="inline-block border-2 border-[#3B1E0E] text-[#3B1E0E] px-8 py-3 rounded-full font-bold hover:bg-[#3B1E0E] hover:text-white transition">{{ __('site.cta.view_menu') }}</a>
            </div>
        </div>
    </section>
    ...
    <section class="bg-[#C8922A] py-16 text-center">
        <div class="max-w-4xl mx-auto px-4">
            ...
            <a href="{{ route('reserve') }}" class="inline-block bg-[#3B1E0E] text-white px-10 py-4 rounded-full font-bold hover:bg-black transition shadow-lg">{{ __('site.cta.book_now') }}</a>
        </div>
    </section>
@endsection
